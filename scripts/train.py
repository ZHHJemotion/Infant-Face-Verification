...
from models.deepid2_resnet import DeepID2_ResNet
...
def train(args):
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5963, 0.4372, 0.3824), (0.2064, 0.1785, 0.1646))
    ])
    ...
    for epoch in np.arange(args.start_epoch, args.num_epochs):
        ...
        for train_i, (labels, images_1, images_2) in enumerate(tra_loader):  # One mini-Batch data, One iteration
            ...
            optimizer.zero_grad()
            feats_1, feats_2, diff_preds = model(images_1, images_2)  # Here we have 3 output
        ...
        while vali_count < num_val:
            for i_val, (labels_val, images_1_val, images_2_val) in enumerate(val_loader):
                ...
                feats_1, feats_2, preds_val = model(images_1_val, images_2_val)  # Here we have 4 output for 4 loss
        ...
        if acc_val >= beat_map:
            ...
            save_dir = os.path.join(weight_dir, "faceVerification_best_model.pkl")
            torch.save(state, save_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--val_pair', nargs='?', type=int, default=32000,
    ...
    parser.add_argument('--verbose_interval', nargs='?', default=50,  # 60
